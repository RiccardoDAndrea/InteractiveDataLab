            ...
            with machine_learning:
                ...
                with Correlation_Matrix:
                    ...
                    with open('Correlation Matrix.png', 'rb') as f:
                        ...
                        st.download_button(label='Download Correlation Matrix', 
                                           data=img_bytes_correlationplot, 
                                           file_name='Barplot.png')
    ...
    ####  M A C H I N E _ L E A R N I N G ###
                ...
                with MachineLearning_sklearn:
                    ...
                    if any(uploaded_file[x].dtype == object for x in X_variables):
                        ...
                        st_lottie(wrong_data_type_ML, 
                                  width = 700, height = 300, quality = 'low', loop = False)
                        st.dataframe(uploaded_file.dtypes, 
                                     use_container_width = True)
                    ...
                    with train_size_col:
                        train_size = st.slider('Train Size', min_value = 0, 
                                               max_value = total_size, 
                                               value = train_size, 
                                               key = 'Sklearn train size')
                    ...
                    elif train_size + test_size > len(uploaded_file):
                    ...
                    R2_sklearn_train = lm.score(X_train,
                                                 y_train)
                    
                    R2_sklearn_test = lm.score(X_test, 
                                               y_test)
                   
                    R_2_training_col,R_2_test_col = st.columns(2)
                    with R_2_training_col:
                        st.metric(label = 'R2 of the Training Data', 
                                  value = R2_sklearn_train.round(3))
                        
                    with R_2_test_col:
                        st.metric(label = 'R2 of the Test Data', 
                                  value = R2_sklearn_test.round(3))
                    ...
                    RMSE_train = sqrt(mean_squared_error(y_train, 
                                                         y_pred_train))
                    RMSE_test = sqrt(mean_squared_error(y_test, 
                                                        y_pred_test))

                    RMSE_train_col,RMSE_test_col = st.columns(2)

                    with RMSE_train_col:
                        st.metric(label = 'RMSE of the Training Data', 
                                  value = round(RMSE_train,3))

                    with RMSE_test_col:
                        st.metric(label = 'RMSE of the Test Data', 
                                  value = round(RMSE_test,3))
                    ...
                    with RMSE_train_col:
                        st.metric(label = 'MAE of the Training Data', 
                                  value = round(MAE_train,3))

                    with RMSE_test_col:
                        st.metric(label = 'MAE of the Test Data', 
                                  value = round(MAE_test,3))

                    # Coefficient
                    coefficiensts = lm.coef_
                    coefficients = pd.DataFrame(lm.coef_.reshape(-1, 1), 
                                                columns=['Coefficient'], 
                                                index=X.columns)
                ...
                with plt_Scatter:
                    ...
                    residuals_plot = px.scatter(results, 
                                                x='y_pred_test', 
                                                y='difference')
                    ...
                    st.plotly_chart(residuals_plot, 
                                    use_container_width = True)
                ...
                with try_machine_learning:
                    ...
                    if len(X.columns) == 1:
                        x0 = st.number_input(X_train.columns[0], 
                                             min_value = user_input_min, 
                                             max_value = user_input_max,
                                             step = 1,
                                             key = 'x1')
                        ...
                        x0 = regression_options_2_1.number_input(X_train.columns[0], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_2_1')
                        
                        x1 = regression_options_2_2.number_input(X_train.columns[1], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,key='regression_options_2_2')
                        ...
                        x0 = regression_options_3_1.number_input(X_train.columns[0], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_3_1')
                        
                        x1 = regression_options_3_2.number_input(X_train.columns[1], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_3_2')
                        
                        x2 = regression_options_3_3.number_input(X_train.columns[2], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key ='regression_options_3_3')
                        ...
                        x0 = regression_options_4_1.number_input(X_train.columns[0], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_4_1')
                        
                        x1 = regression_options_4_2.number_input(X_train.columns[1], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_4_2')
                        
                        x2 = regression_options_4_3.number_input(X_train.columns[2], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_4_3')
                        
                        x3 = regression_options_4_4.number_input(X_train.columns[3], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_4_4')
                        ...
                        x0 = regression_options_5_1.number_input(X_train.columns[0], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_5_1')
                        
                        x1 = regression_options_5_2.number_input(X_train.columns[1], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_5_2')
                        
                        x2 = regression_options_5_3.number_input(X_train.columns[2], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_5_3')

                        regression_options_5_4,regression_options_5_5 = st.columns(2)
                       
                        x3 = regression_options_5_4.number_input(X_train.columns[3], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_5_4')
                        
                        x4 = regression_options_5_5.number_input(X_train.columns[4], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,key = 'regression_options_5_5')
                        ...
                        x0 = regression_options_6_1.number_input(X_train.columns[0], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_6_1')
                        
                        x1 = regression_options_6_2.number_input(X_train.columns[1], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_6_2')
                        
                        x2 = regression_options_6_3.number_input(X_train.columns[2], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_6_3')
                        
                        regression_options_6_4,regression_options_6_5,regression_options_6_6 = st.columns(3)
                        
                        x3 = regression_options_6_4.number_input(X_train.columns[3], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_6_4')
                        
                        x4 = regression_options_6_5.number_input(X_train.columns[4], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regregression_options_6_5')
                        
                        x5 = regression_options_6_6.number_input(X_train.columns[5], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_6_6')
                        ...
                        x0 = regression_options_7_1.number_input(X_train.columns[0], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_7_1')
                        
                        x1 = regression_options_7_2.number_input(X_train.columns[1], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_7_2')
                        x2 = regression_options_7_3.number_input(X_train.columns[2], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_7_3')
                        
                        regression_options_7_4,regression_options_7_5,regression_options_7_6,regression_options_7_7 = st.columns(4)
                        
                        x3 = regression_options_7_4.number_input(X_train.columns[3], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_7_4')
                        
                        x4 = regression_options_7_5.number_input(X_train.columns[4], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key='regregression_options_7_5')
                        
                        x5 = regression_options_7_6.number_input(X_train.columns[5], 
                                                                 min_value = user_input_min,
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_7_6')
                        
                        x6 = regression_options_7_7.number_input(X_train.columns[6], 
                                                                 min_value = user_input_min, 
                                                                 max_value = user_input_max,
                                                                 step = 1,
                                                                 key = 'regression_options_7_7')
